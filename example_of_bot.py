...
def turn_handler(
    ...
    if true_out_response is not None and true_out_response != out_response:
        ...
        logger.info(out_response)
        logger.info("response must be")
        logger.info(true_out_response)
        msg = f"in_request={in_request} -> true_out_response != out_response: {true_out_response} != {out_response}"
        raise Exception(msg)
    else:
        logger.info(f"in_request={in_request} -> NODE {list(ctx.labels.values())[-1]} RESPONSE {out_response}")
    ...
    for in_request, true_out_response in testing_dialog:
        ...
        if mode == "json":
            ...
            if isinstance(ctx, str):
                logger.info("context serialized to json str")
            else:
                raise Exception(f"ctx={ctx} has to be serialized to json string")
        elif mode == "dict":
            ctx = ctx.dict()
            if isinstance(ctx, dict):
                logger.info("context serialized to dict")
            else:
