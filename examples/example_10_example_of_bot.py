from df_engine.core.keywords import GLOBAL, TRANSITIONS, RESPONSE,MISC,LOCAL,PRE_RESPONSE_PROCESSING,PRE_TRANSITIONS_PROCESSING
...
def add_misc():
    def add_misc_processing(ctx: Context, actor: Actor, *args, **kwargs) -> Context:
        ...
        if not callable(processed_node.response):
            ...
            processed_node.response = f"misc: {processed_node.misc} {processed_node.response(ctx, actor, *args, **kwargs)}"    
        ctx.overwrite_current_node_in_processing(processed_node)
        return ctx
​
...
def fallback_trace_response(ctx: Context, actor: Actor, *args, **kwargs) -> Any:
    ...
    return str({"previous_node": list(ctx.labels.values())[-2], "last_request": ctx.last_request})
​
​
...
script = {
    GLOBAL:{TRANSITIONS:{("global_flow","start_node",1.5):cnd.exact_match("global start"),
            ...
            PRE_RESPONSE_PROCESSING: {"proc_name_1": add_prefix("l1_global"),"proc_name_2": add_prefix("l2_global")},
    ...
    "flow": { LOCAL:{  MISC: {
        ...
        "node_no": {MISC:{"var3":"rewrite_by_NO"},
                   ...
                   TRANSITIONS:{ ("flow","node_complex"):complex_user_answer_condition,
...
# edit this dialog
testing_dialog = [
    ("base", "misc: {'var1': 'global_data', 'var2': 'rewrite_by_flow', 'var3': 'rewrite_by_hi'} response: l2_flow_hi: l1_flow_hi: Hi!!!"),  # start_node -> node1
    ("no", "misc: {'var1': 'global_data', 'var2': 'rewrite_by_flow', 'var3': 'rewrite_by_NO'} response: l2_flow_no: l1_flow_no: NO"),  # node1 -> node2
    ("talk about books", "misc: {'var1': 'global_data', 'var2': 'rewrite_by_flow', 'var3': 'rewrite_by_TOPIC'} response: l2_flow_topic: l1_flow_topic:"),  # node3 -> node4
    ("ok", "misc: {'var1': 'global_data', 'var2': 'rewrite_by_flow', 'var3': 'rewrite_by_OK'} response: l2_flow_ok: l1_flow_ok: "),  # node4 -> node1
    ("previous", "NODE ('flow', 'node_topic') RESPONSE misc: {'var1': 'global_data', 'var2': 'rewrite_by_flow', 'var3': 'rewrite_by_TOPIC'} response: l2_flow_topic: l1_flow_topic: "),  # node1 -> fallback_node
    ("{1:2}", "misc: {'var1': 'global_data', 'var2': 'rewrite_by_flow', 'var3': 'rewrite_by_COMPLEX'} response: l2_flow_complex: l1_flow_complex:"),  # fallback_node -> fallback_node
    ("f", "misc: {'var1': 'global_data', 'var2': 'global_data', 'var3': 'global_data'} response: l2_global: l1_global: oops ")
]
​
​
